...
Convenção: os arquivos em `assets/img/...` já existem com o nome final que devem ter; enquanto o conteúdo real não é exportado do Framer, o arquivo contém um placeholder simples (SVG com rótulo). A lista do que falta exportar/substituir está em `MANUTENCAO.md`.

## Conteúdo com baixa confiança de leitura

O texto completo dos depoimentos e a lista exata de logos em "empresas que já apoiaram" não deram para ler com confiança no screenshot de referência (`plan/framer-version.png`) — ficaram como placeholder/genérico em `_data/depoimentos.yml` e `_data/apoiadores.yml`. Precisam ser confirmados por quem tem acesso ao Framer antes de publicar de verdade.
